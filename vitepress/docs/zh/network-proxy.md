...
规则集、订阅的定时刷新，以及 **策略组测速 / 出口国家探测 / SSH 能力探测 / 服务可达探测** 等全部周期任务，统一由 **计划任务** 调度（侧栏 **系统 → 计划任务**）。周期跟随对应配置（组的测速间隔、geo/reach 的重测周期等）：队列里能看到每个任务的 **周期 / 下次执行倒计时 / 上次执行结果**，失败会显示**失败原因**；点 **「立即执行」** 可手动唤醒，选中任务可查看最近的**执行历史**（时间 / 成败 / 原因 / 耗时）。任务状态持久化在本地，daemon 重启后按原计划恢复。
